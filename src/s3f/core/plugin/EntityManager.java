...
public class EntityManager {
    ...
    public <T> List<T> getAllProperties(String path, String field, Class<? extends T> filter) {
        ArrayList<Data> result = new ArrayList<>();
        Data.search(path.split("\\."), result, treeRoot);
        if (result.isEmpty()) {
            return null;
        } else {
            ArrayList<T> properties = new ArrayList<>();
            for (Data data : result) {
                Object o = data.getProperty(field);
    ...
    public <T> List<T> getEntities(String path, Class<? extends T> filter) {
        List<Data> allData = getAllData(path);
        ArrayList<T> result = new ArrayList<>();
        if (allData != null) {
            for (Data data : allData) {
                Object o = data.getReference();
                if (filter.isInstance(o)) {
                    T t = (T) o;
                    result.add(t);
                }
    ...
    public <T> List<T> getAllEntities(Class<? extends T> filter) {
